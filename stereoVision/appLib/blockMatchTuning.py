...
import appLib.camera as camera
from appLib.stereoCalibration import load_stereo_map

class blockMatchTuning:
    def __init__(self, config):
        if (not e.is_raspberry_pi()):
            raise EnvironmentError("blockMatchTuning is intended to run on Raspberry Pi only.")
        self.config = config
    ...
    def run(self):
        ...
        if imgL is not None:
            ...
            for M in (self.Left_Stereo_Map_x, self.Left_Stereo_Map_y, self.Right_Stereo_Map_x, self.Right_Stereo_Map_y):
        ...
        ibL = np.mean((self.Left_Stereo_Map_x >= 0) & (self.Left_Stereo_Map_x <= w-1) & (self.Left_Stereo_Map_y >= 0) & (self.Left_Stereo_Map_y <= h-1))
        ibR = np.mean((self.Right_Stereo_Map_x >= 0) & (self.Right_Stereo_Map_x <= w-1) & (self.Right_Stereo_Map_y >= 0) & (self.Right_Stereo_Map_y <= h-1))
